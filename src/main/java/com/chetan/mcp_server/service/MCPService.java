...
// Threshold for malicious
    ...
    public void fetchAndSaveJiraIssues() {
        try {
            String searchUrl = jiraUrl.replace("/issue", "/search")
                    + "?jql=project=" + jiraProjectKey + " ORDER BY created DESC&maxResults=10";
            ...
            if (body != null && body.containsKey("issues")) {
                ...
                for (Map<String, Object> issue : issues) {
                    ...
                    if (!jiraRepo.existsById(issueId)) {
                        ...
                        // ✅ Only check entities for new issue
                        checkEntitiesForIssue(newIssue);
                    }
                }
            }
        } catch (Exception e) {
            logger.error("❌ Jira fetch error: {}", e.getMessage());
        }
    }
